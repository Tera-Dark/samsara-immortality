                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-6 p-6 bg-white/60 border border-slate-200 rounded-2xl backdrop-blur-md transition-all duration-300 hover:bg-slate-100/60 hover:border-emerald-400/30">
                        ...
                        <div className="shrink-0 relative">
                            ...
                            <div className="absolute -bottom-2 w-full text-center">
                                <span className="bg-slate-50 px-3 py-1 text-sm text-slate-500 font-mono border border-slate-200 rounded text-outlined shadow-sm">
                                    Lv.{index}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 text-center md:text-left">
                            <h3 className="text-3xl font-serif font-bold tracking-widest mb-1 text-slate-700 group-hover:text-emerald-600 transition-colors text-outlined-strong">
                                {realm.name}
                            </h3>
                            <p className="text-base text-slate-500 italic mb-2 font-serif opacity-80 text-outlined">"{realm.flavor}"</p>
                            <p className="text-base text-slate-600 leading-relaxed mb-4 text-outlined">{realm.desc}</p>

                            {/* Sub-realms */}
                            {realm.subRealms && realm.subRealms.length > 0 && (
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {realm.subRealms.map(sub => (
                                        <span key={sub} className="text-sm px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 shadow-sm font-serif">
                                            {sub}
                                        </span>
                                    ))}
                                </div>
                            ...
                            {/* Modifiers Grid */}
                            <div className="grid grid-cols-2 gap-3 text-base text-slate-500 bg-slate-50 p-4 rounded-xl border border-slate-200">
                        ...
                        <div className="shrink-0 w-full md:w-36 text-center md:border-l md:border-slate-200 md:pl-4">
                            <div className="text-base font-serif text-slate-500 mb-2 text-outlined border-b border-slate-200/50 pb-1">突破条件</div>
                            {realm.breakthrough.probability && (
                                <div className="text-base mb-3 text-outlined">
                                    成功率: <span className="text-amber-600 font-bold">{realm.breakthrough.probability * 100}%</span>
                                </div>
                            )}
                            {realm.breakthrough.stats && (
                                <div className="flex flex-col gap-1 text-sm text-slate-500 text-outlined font-mono">
                                    {Object.entries(realm.breakthrough.stats).map(([k, v]) => (
                                        <span key={k} className="bg-white/50 rounded px-1">{k} &ge; {v}</span>
                                    ))}
                                </div>
                            )}
                            {realm.breakthrough.items && (
                                <div className="mt-3 pt-3 border-t border-slate-200">
                                    <div className="text-sm text-slate-500 text-outlined mb-1">所需丹药:</div>
                                    {realm.breakthrough.items.map(item => (
                                        <div key={item} className="text-sm text-rose-600 font-bold text-outlined">{item}</div>
                                    ))}
                                </div>
                            )}
                        </div>
